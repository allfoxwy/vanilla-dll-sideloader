...
static LPSTR ReadFileIntoMemory(HANDLE hFile) {
	...
	LPSTR readBuffer = static_cast<LPSTR>(malloc(readBufferSize + 1));
...
LPWSTR* FromTextFile(LPCWSTR pFileName, int* nLinesRead, BOOL ignoreComments) {
	...
	*nLinesRead = 0;
	...
	LPSTR* pLines = static_cast<LPSTR*>(calloc(4, sizeof(LPSTR)));
	int pMaxLines = 4;
	...
	while(*pCurrent) {
		...
		if((!isComment || !ignoreComments) && !isEmptyLine) {
			...
			if(*nLinesRead >= pMaxLines) {
				pMaxLines *= 2;
				pLines = static_cast<LPSTR*>(realloc(pLines, pMaxLines * sizeof(LPSTR)));
			}
	...
	pResult = static_cast<LPWSTR*>(calloc(*nLinesRead, sizeof(LPWSTR)));
	for(int i = 0; i < *nLinesRead; i++) {
		...
		pResult[i] = static_cast<LPWSTR>(malloc(chars * sizeof(WCHAR)));
...
BOOL ToTextFile(LPCWSTR pFileName, LPWSTR* pLines, int nLines) {
	...
	for(int i = 0; i < nLines; i++) {
		...
		LPSTR writeBuffer = static_cast<LPSTR>(calloc(chars, 1));
